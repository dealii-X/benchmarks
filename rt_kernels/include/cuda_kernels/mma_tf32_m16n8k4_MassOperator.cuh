...
__device__ __forceinline__ uint32_t float_to_tf32(float f) {
    uint32_t res;
    // cvt: convert
    // rna: round to nearest, ties to even
    // tf32.f32: from 32-bit float to tf32 format
...
__device__ void tf32_m16n8k4_tiled_gemm(float *s_A, float *s_B, float *s_C)
{   
    constexpr int m = 16;
    constexpr int n = 8;
    constexpr int k = 4;
    ...
    uint32_t r_b[num_tiles_k][num_tiles_n] = {0};

    auto s_B_view = matrixView<float, Layout_B, K, N>(s_B);
    ...
    for(int i = 0; i < num_tiles_k; i++){
        ...
        for(int j = 0; j < num_tiles_n; j++){
            col = base_col + j * n;
            if(row < K && col < N){
                r_b[i][j] = float_to_tf32(s_B_view(row, col));
            } 
            else{
                r_b[i][j] = 0;
            }
    ...
    auto s_A_view = matrixView<float, Layout_A, M, K>(s_A);

    uint32_t r_a[num_tiles_m][num_tiles_k][2] = {0};
    float r_c[num_tiles_m][num_tiles_n][4] = {0};
    ...
    //copy s_A from shared memory to register
    {
        ...
        for(int i = 0; i < num_tiles_m; i++){
            ...
            for(int j = 0; j < num_tiles_k; j++){
                col = base_col + j * k;
                if(row0 < M && col < K){
                    r_a[i][j][0] = float_to_tf32(s_A_view(row0, col));
                }

                if(row1 < M && col < K){
                    r_a[i][j][1] = float_to_tf32(s_A_view(row1, col));
                }
    ...
    auto s_C_view = matrixView<float, Layout_C, M, N>(s_C);

    {
        //copy from register to shared memory s_C
        const int base_row = laneid >> 2;

        const int base_col0 = ((laneid % 4) * 2) + (0 & 0x1);
        const int base_col1 = ((laneid % 4) * 2) + (1 & 0x1);
        ...
        for(int j = 0; j < num_tiles_n; ++j){
            ...
            for(int i = 0; i < num_tiles_m; ++i){
                ...
                if(row0 < M){
                    if(col0 < N){
                        s_C_view(row0, col0) = r_c[i][j][0];
                    }
                    if(col1 < N){
                        s_C_view(row0, col1) = r_c[i][j][1];
                    }
                }

                if(row1 < M){
                    if(col0 < N){
                        s_C_view(row1, col0) = r_c[i][j][2];
                    }
                    if(col1 < N){
                        s_C_view(row1, col1) = r_c[i][j][3];
                    }
                }    
            }
        }
...
void __global__ f32_m16n8k4_Mass_mma(
    const unsigned int nelmt,
    const unsigned int numBlocks, const unsigned int threadsPerBlock,
    const float *__restrict__ d_basis_n, const float *__restrict__ d_basis_t,
    const float *__restrict__ d_G, float *__restrict__ d_in, float * __restrict__ d_out, const unsigned int ntests) 
{
    using T = float;
    ...
    while(eb < (nelmt + nelmtPerBatch - 1) / nelmtPerBatch)
    {
        ...
        tf32_m16n8k4_tiled_gemm<nelmtPerBatch * nm_n * nm_t, nq, nm_t, Layout::RowMajor, Layout::RowMajor, Layout::RowMajor>(s_uq_0, s_basis_t, s_wsp0);
        ...
        tf32_m16n8k4_tiled_gemm<nelmtPerBatch * nm_n * nq, nq, nm_t, Layout::RowMajor, Layout::RowMajor, Layout::RowMajor>(s_wsp1, s_basis_t, s_wsp0);
        ...
        tf32_m16n8k4_tiled_gemm<nelmtPerBatch * nq * nq, nq, nm_n, Layout::RowMajor, Layout::RowMajor, Layout::RowMajor>(s_wsp1, s_basis_n, s_uq_0);
        ...
        tf32_m16n8k4_tiled_gemm<nelmtPerBatch * nm_n * nm_t, nq, nm_t, Layout::RowMajor, Layout::RowMajor, Layout::RowMajor>(s_uq_1, s_basis_t, s_wsp0);
        ...
        //s_wsp1(eirj) . s_basis_t(jq) = s_wsp0(eirq)
        tf32_m16n8k4_tiled_gemm<nelmtPerBatch * nm_t * nq, nq, nm_n, Layout::RowMajor, Layout::RowMajor, Layout::RowMajor>(s_wsp1, s_basis_n, s_wsp0);
        ...
        tf32_m16n8k4_tiled_gemm<nelmtPerBatch * nq * nq, nq, nm_t, Layout::RowMajor, Layout::RowMajor, Layout::RowMajor>(s_wsp1, s_basis_t, s_uq_1);
        ...
        tf32_m16n8k4_tiled_gemm<nelmtPerBatch * nm_t * nm_t, nq, nm_n, Layout::RowMajor, Layout::RowMajor, Layout::RowMajor>(s_uq_2, s_basis_n, s_wsp0);
        ...
        //s_wsp1(eirj) . s_basis_t(jq) = s_wsp0(eirq)
        tf32_m16n8k4_tiled_gemm<nelmtPerBatch * nm_t * nq, nq, nm_t, Layout::RowMajor, Layout::RowMajor, Layout::RowMajor>(s_wsp1, s_basis_t, s_wsp0);
        ...
        //s_wsp1(erqi) . s_basis_n(ip) = s_uq_1(erqp)
        tf32_m16n8k4_tiled_gemm<nelmtPerBatch * nq * nq, nq, nm_t, Layout::RowMajor, Layout::RowMajor, Layout::RowMajor>(s_wsp1, s_basis_t, s_uq_2);
        ...
        tf32_m16n8k4_tiled_gemm<nelmtPerBatch * nq * nq, nm_n, nq, Layout::RowMajor, Layout::ColMajor, Layout::RowMajor>(s_uq_0, s_basis_n, s_wsp0);
        ...
        tf32_m16n8k4_tiled_gemm<nelmtPerBatch * nq * nm_n, nm_t, nq, Layout::RowMajor, Layout::ColMajor, Layout::RowMajor>(s_wsp1, s_basis_t, s_wsp0);
        ...
        tf32_m16n8k4_tiled_gemm<nelmtPerBatch * nm_t * nm_n, nm_t, nq, Layout::RowMajor, Layout::ColMajor, Layout::RowMajor>(s_wsp1, s_basis_t, s_uq_0);
        ...
        tf32_m16n8k4_tiled_gemm<nelmtPerBatch * nq * nq, nm_t, nq, Layout::RowMajor, Layout::ColMajor, Layout::RowMajor>(s_uq_1, s_basis_t, s_wsp0);
        ...
        tf32_m16n8k4_tiled_gemm<nelmtPerBatch * nq * nm_t, nm_n, nq, Layout::RowMajor, Layout::ColMajor, Layout::RowMajor>(s_wsp1, s_basis_n, s_wsp0);
        ...
        tf32_m16n8k4_tiled_gemm<nelmtPerBatch * nm_t * nm_n, nm_t, nq, Layout::RowMajor, Layout::ColMajor, Layout::RowMajor>(s_wsp1, s_basis_t, s_uq_1);
        ...
        tf32_m16n8k4_tiled_gemm<nelmtPerBatch * nq * nq, nm_t, nq, Layout::RowMajor, Layout::ColMajor, Layout::RowMajor>(s_uq_2, s_basis_t, s_wsp0);
        ...
        //s_wsp1(eriq) . s_basis_n(qj) = s_wsp0(erij)
        tf32_m16n8k4_tiled_gemm<nelmtPerBatch * nq * nm_t, nm_t, nq, Layout::RowMajor, Layout::ColMajor, Layout::RowMajor>(s_wsp1, s_basis_t, s_wsp0);
        ...
        tf32_m16n8k4_tiled_gemm<nelmtPerBatch * nm_t * nm_t, nm_n, nq, Layout::RowMajor, Layout::ColMajor, Layout::RowMajor>(s_wsp1, s_basis_n, s_uq_2);
...
#endif //MMA_TF32_M16N8K4_MASS_OPERATOR_CUH
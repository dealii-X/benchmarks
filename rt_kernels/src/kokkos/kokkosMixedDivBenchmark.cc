...
#include "kokkosMixedDivOperator.hpp"
#include <timer.hpp>
...
{   
    ...
    std::fill(in_u,  in_u + nelmt * ndof_u_total, (T)3.0f);
    std::fill(out_p, out_p + nelmt * ndof_p_total, (T)0.0f);
    ...
    // ------------------------- MixedDiv Operator Kernel -----------------------------------------------
    {
        std::vector<double> results = Parallel::Kokkos_MixedDiv<T, nq, nm_t, nm_n, nm_p>(
            nelmt, nelmtPerBatch, numBlocks, threadsPerBlock, 
            dbasis_n, basis_t, basis_p, G_scalar, in_u, out_p, ntests
        );
        ...
        T bw = 1.0e-9 * (nDOF_U + nDOF_P) * sizeof(T) / time; 
        
        printer("MixedDiv", nq - 2, nelmt, nelmtPerBatch, numBlocks, threadsPerBlock, nDOF_U, time, DOFs, bw, std::sqrt(sum));
    }

    delete[] dbasis_n; delete[] basis_t; delete[] basis_p; delete[] G_scalar; delete[] in_u; delete[] out_p;
}
...
int main(int argc, char **argv){
    ...
    const size_t shmemPerBlock = 10000;    //maximum shared memory size per thread block

    unsigned int p      = (argc > 1) ? atoi(argv[1]) : 2u;    
    unsigned int nq     = p + 2;
    unsigned int nelmt  = (argc > 2) ? atoi(argv[2]) : 2 << 15;
    
    unsigned int nelmtPerBatch = (argc > 3) ? atoi(argv[3]) : std::max(1UL, shmemPerBlock / (3 * nq * nq * nq) / sizeof(T));

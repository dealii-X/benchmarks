...
namespace BK3
{
  namespace Serial
  {
    ...
    template <typename T>
    T
    SumFactorization(const unsigned int nq0,
                  ...
                  for (unsigned int r = 0; r < nq2; ++r)
                    {
                      ...
                      Grr = G[e * nq0 * nq1 * 6 * nq2 + p * nq1 * 6 * nq2 +
                              q * 6 * nq2 + 0 * nq2 + r];
                      Grs = G[e * nq0 * nq1 * 6 * nq2 + p * nq1 * 6 * nq2 +
                              q * 6 * nq2 + 1 * nq2 + r];
                      Grt = G[e * nq0 * nq1 * 6 * nq2 + p * nq1 * 6 * nq2 +
                              q * 6 * nq2 + 2 * nq2 + r];
                      Gss = G[e * nq0 * nq1 * 6 * nq2 + p * nq1 * 6 * nq2 +
                              q * 6 * nq2 + 3 * nq2 + r];
                      Gst = G[e * nq0 * nq1 * 6 * nq2 + p * nq1 * 6 * nq2 +
                              q * 6 * nq2 + 4 * nq2 + r];
                      Gtt = G[e * nq0 * nq1 * 6 * nq2 + p * nq1 * 6 * nq2 +
                              q * 6 * nq2 + 5 * nq2 + r];
                      ...
                      for (unsigned int n = 0; n < nq0; ++n)
                        {
                          qr += wsp1[n * nq1 * nq2 + q * nq2 + r] *
                                dbasis0[n * nq0 + p];
                        }

                      for (unsigned int n = 0; n < nq1; ++n)
                        {
                          qs += wsp1[p * nq1 * nq2 + n * nq2 + r] *
                                dbasis1[n * nq1 + q];
                        }

                      for (unsigned int n = 0; n < nq2; ++n)
                        {
                          qt += wsp1[p * nq1 * nq2 + q * nq2 + n] *
                                dbasis2[n * nq2 + r];
                        }
                      ...
                      rqr[e * nq0 * nq1 * nq2 + p * nq1 * nq2 + q * nq2 + r] =
                        Grr * qr + Grs * qs + Grt * qt;
                      rqs[e * nq0 * nq1 * nq2 + p * nq1 * nq2 + q * nq2 + r] =
                        Grs * qr + Gss * qs + Gst * qt;
                      rqt[e * nq0 * nq1 * nq2 + p * nq1 * nq2 + q * nq2 + r] =
                        Grt * qr + Gst * qs + Gtt * qt;
                        
                      std::cout << rqr[e * nq0 * nq1 * nq2 + p * nq1 * nq2 +
                                       q * nq2 + r]
                                << " , "
                                << rqs[e * nq0 * nq1 * nq2 + p * nq1 * nq2 +
                                       q * nq2 + r]
                                << " , "
                                << rqt[e * nq0 * nq1 * nq2 + p * nq1 * nq2 +
                                       q * nq2 + r]
                                << " , ";
                      std::cout << std::endl;
                    }
                  ...
                  for (unsigned int r = 0; r < nq2; ++r)
                    {
                      T tmp0 = (T)0;
                      for (unsigned int n = 0; n < nq0; ++n)
                        tmp0 += rqr[e * nq0 * nq1 * nq2 + n * nq1 * nq2 +
                                    q * nq2 + r] *
                                dbasis0[p * nq0 + n];

                      for (unsigned int n = 0; n < nq1; ++n)
                        tmp0 += rqs[e * nq0 * nq1 * nq2 + p * nq1 * nq2 +
                                    n * nq2 + r] *
                                dbasis1[q * nq1 + n];

                      for (unsigned int n = 0; n < nq2; ++n)
                        tmp0 += rqt[e * nq0 * nq1 * nq2 + p * nq1 * nq2 +
                                    q * nq2 + n] *
                                dbasis2[r * nq2 + n];

#ifndef BK5_KOKKOS_KERNELS_HPP
#define BK5_KOKKOS_KERNELS_HPP
                    ...
                    KOKKOS_LAMBDA (member_type team_member){
                        ...
                        {   
                            for(unsigned int tid = threadIdx; tid < nq0 * nq1 * nq2; tid += blockSize){
                                ...
                                unsigned int i = tid / (nq1 * nq2);
                                unsigned int j = (tid % (nq1 * nq2)) / nq2;
...
template <typename T>
std::vector<T> KokkosKernel_2D_Block_jk(const unsigned int nq0, const unsigned int nq1, const unsigned int nq2,
    const T *__restrict__ dbasis0, const T *__restrict__ dbasis1, const T *__restrict__ dbasis2,
    const T* __restrict__ G, const T* __restrict__ in, T* __restrict__ out,
    const unsigned int numThreads, const unsigned int threadsPerBlock, const unsigned int nelmt, const unsigned int ntests)
    {   
        ...
        {   
            ...
            typedef Kokkos::TeamPolicy<>::member_type member_type;
            Kokkos::TeamPolicy<> policy(numBlocks, (std::min(nq1 * nq2, threadsPerBlock / nq0)));
            policy.set_scratch_size(0, Kokkos::PerTeam(shmem_size));
            ...
            for (unsigned int t = 0u; t < ntests; ++t)
            {
                ...
                Kokkos::parallel_for(policy,
                    KOKKOS_LAMBDA (member_type team_member){
                        ...
                        while(e < nelmt)
                        {   
                            for(unsigned int tid = threadIdx; tid < nq1 * nq2; tid += blockSize){
...
template <typename T>
std::vector<T> KokkosKernel_2D_Block_jk_SimpleMap(const unsigned int nq0, const unsigned int nq1, const unsigned int nq2,
    const T *__restrict__ dbasis0, const T *__restrict__ dbasis1, const T *__restrict__ dbasis2,
    const T* __restrict__ G, const T* __restrict__ in, T* __restrict__ out,
    const unsigned int numThreads, const unsigned int nelmt, const unsigned int ntests)
    {   

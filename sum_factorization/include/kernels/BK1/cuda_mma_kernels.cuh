...
__device__ auto matrixView(T* data, const size_t rows, const size_t cols) {
...
__device__ void batched_tiled_gemm(double *s_batched_A, double *s_B, double *s_batched_C,
                       const int num_batch, const int M, const int N, const int K)
{
    ...
    auto s_B_view = matrixView<double, Layout_B>(s_B, K, N);
    ...
    for(int batch_id = 0; batch_id < num_batch; batch_id++)
    {
        ...
        auto current_s_A_view = matrixView<double, Layout_A>(s_batched_A + batch_id * (M * K), M, K);
        ...
        auto current_s_C_view = matrixView<double, Layout_C>(s_batched_C + batch_id * (M * N), M, N);
...
void __global__ BwdTransHexKernel_mma(const unsigned int nq0, const unsigned int nq1, const unsigned int nq2,
    ...
    const T *__restrict__ d_basis2, const T *__restrict__ d_JxW, const T *__restrict__ d_in, T *__restrict__ d_out) 
{
    ...
    while(e < nelmt)
    {
        ...
        batched_tiled_gemm<m, n, t, Layout::ColMajor, Layout::ColMajor, Layout::RowMajor>(s_wsp1, s_basis1, s_wsp0, nq0, nm2, nq1, nm1);

        //step-4 : direction 2      s_wsp0(pkq) s_basis2(rk) -> s_wsp1(pqr)
        batched_tiled_gemm<m, n, t, Layout::ColMajor, Layout::ColMajor, Layout::RowMajor>(s_wsp0, s_basis2, s_wsp1, nq0, nq1, nq2, nm2);
        ...
        batched_tiled_gemm<m, n, t, Layout::RowMajor, Layout::RowMajor, Layout::RowMajor>(s_wsp1, s_basis2, s_wsp0, nq0, nq1, nm2, nq2);

        //step-7 : direction 1      s_wsp0(pqk) s_basis1(qj) -> s_wsp1(pkj)
        batched_tiled_gemm<m, n, t, Layout::ColMajor, Layout::RowMajor, Layout::RowMajor>(s_wsp0, s_basis1, s_wsp1, nq0, nm2, nm1, nq1);

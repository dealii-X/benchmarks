...
template <int dim, int fe_degree, int nq>
...
private:
  ...
  std::unique_ptr<Portable::LaplaceOperator<dim, fe_degree, nq, double>>
    system_matrix;
...
template <int dim, int fe_degree, int nq>
LaplaceProblem<dim, fe_degree, nq>::LaplaceProblem()
...
template <int dim, int fe_degree, int nq>
void
LaplaceProblem<dim, fe_degree, nq>::setup_dofs()
{
  Timer time;
...
template <int dim, int fe_degree, int nq>
void
LaplaceProblem<dim, fe_degree, nq>::setup_matrix_free()
{
  Kokkos::fence();
  Timer time;

  system_matrix =
    std::make_unique<Portable::LaplaceOperator<dim, fe_degree, nq, double>>(
...
template <int dim, int fe_degree, int nq>
void
LaplaceProblem<dim, fe_degree, nq>::compute_rhs()
{
  Kokkos::fence();
  ...
  const QGauss<dim> quadrature_formula(nq);
...
template <int dim, int fe_degree, int nq>
void
LaplaceProblem<dim, fe_degree, nq>::do_matvec()
{
  system_matrix->vmult(solution_device, system_rhs_device);
}
template <int dim, int fe_degree, int nq>
void
LaplaceProblem<dim, fe_degree, nq>::solve()
{
  Kokkos::fence();
  Timer time;
...
template <int dim, int fe_degree, int nq>
void
LaplaceProblem<dim, fe_degree, nq>::matvec_ghost_timing()
{
...
template <int dim, int fe_degree, int nq>
void
LaplaceProblem<dim, fe_degree, nq>::postprocess_solution()
{
  LinearAlgebra::ReadWriteVector<double> rw_vector(locally_owned_dofs);
...
template <int dim, int fe_degree, int nq>
void
LaplaceProblem<dim, fe_degree, nq>::run(const std::size_t min_size,
...
public:
                 ...
                 const std::size_t  max_size)
  {
    ...
    if (min_degree == target_degree)
      {
        constexpr int nq = min_degree + 2;
        LaplaceProblem<dim, min_degree, nq> laplace_problem;
      ...
      if (argc == 1)
        {
          if (Utilities::MPI::this_mpi_process(MPI_COMM_WORLD) == 0)
            std::cout << "Expected at least one argument." << std::endl
                      << "Usage:" << std::endl
                      << "./bp3 degree minsize maxsize" << std::endl;
          return 1;
        }

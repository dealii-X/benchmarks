...
/*template <typename T>
auto bk1_expected_norm2(const int nq0, const int nq0_pad, const float *basis0,
                        const float *basis1, const float *basis2,
                        const float JxW, const float in) {
    const int nq1 = nq0;
    const int nq2 = nq0;
    const int nq1_pad = nq0_pad;
    const int nq2_pad = nq0_pad;
    auto wsp0 = std::vector<T>(nq0 * nq1 * nq2);
    auto rqs = std::vector<T>(nq0 * nq1 * nq2);
    auto rqt = std::vector<T>(nq0 * nq1 * nq2);
            ...
            for (unsigned int j = 0; j < nm1; j++) {
                for (unsigned int i = 0; i < nm0; i++) {
                    wsp1[p * nm1 * nm2 + j * nm2 + k] +=
                        wsp0[i * nm1 * nm2 + j * nm2 + k] * basis0[p * nm0 + i];
                }
            }
        }
    }
        ...
        for (unsigned int j = 0; j < nq1; ++j) {
            for (unsigned int k = 0; k < nq2; ++k) {
                ...
                for (unsigned int n = 0; n < nq0; ++n) {
                    qr += in * dbasis0[i * nq0_pad + n];
                }

                for (unsigned int n = 0; n < nq1; ++n) {
                    qs += in * dbasis1[j * nq1_pad + n];
                }

                for (unsigned int n = 0; n < nq2; ++n) {
                    qt += in * dbasis2[k * nq2_pad + n];
                }
                rqr[i * nq1 * nq2 + j * nq2 + k] = G * qr + G * qs + G * qt;
                rqs[i * nq1 * nq2 + j * nq2 + k] = G * qr + G * qs + G * qt;
                rqt[i * nq1 * nq2 + j * nq2 + k] = G * qr + G * qs + G * qt;
            }
        }
    }
    ...
    for (unsigned int i = 0; i < nq0; ++i) {
        for (unsigned int j = 0; j < nq1; ++j) {
            for (unsigned int k = 0; k < nq2; ++k) {
                T tmp0 = (T)0;
                for (unsigned int n = 0; n < nq2; ++n)
                    tmp0 += rqt[i * nq1 * nq2 + j * nq2 + n] *
                            dbasis2[n * nq2_pad + k];
                for (unsigned int n = 0; n < nq1; ++n)
                    tmp0 += rqs[i * nq1 * nq2 + n * nq2 + k] *
                            dbasis1[n * nq1_pad + j];
                for (unsigned int n = 0; n < nq0; ++n)
                    tmp0 += rqr[n * nq1 * nq2 + j * nq2 + k] *
                            dbasis0[n * nq0_pad + i];
